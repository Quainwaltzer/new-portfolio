...
`;
	...
	function smoothDampVec2(curr, target, vel, smoothTime, deltaTime) {
	...
	function smoothDampFloat(curr, target, velRef, smoothTime, deltaTime) {
		smoothTime = Math.max(0.0001, smoothTime);
		const omega = 2 / smoothTime;
		const x = omega * deltaTime;
		const exp = 1 / (1 + x + 0.48 * x * x + 0.235 * x * x * x);
	...
	function centroid(pts) { let x = 0, y = 0; const n = pts.length || 1; for (const p of pts) { x += p.x; y += p.y; } return { x: x / n, y: y / n }; }
	...
	onMount(() => {
		...
		const onClick = async () => {
			...
			if (current.enableGyro && typeof window !== 'undefined') {
				...
				if (D && D.requestPermission) { try { await D.requestPermission(); } catch (err) { } }
			}
		};
		...
		(async function startFace() {
			...
			const detect = async (ts) => {
				...
				if (ts - lastDetect >= 33) {
					...
					} catch (err) { uiFaceActive = false; }
				}
				if (videoRef && videoRef.requestVideoFrameCallback) {
					videoRef.requestVideoFrameCallback(() => detect(performance.now()));
				} else {
		...
		const tick = () => {
			...
			uniforms.uLinesColor.value.copy(srgbColor(current.linesColor));
			uniforms.uScanColor.value.copy(srgbColor(current.scanColor));
			...
			if (bloom) {
				...
				bloom.luminanceMaterial.smoothing = current.bloomSmoothing;
			}
...
<div bind:this={containerRef} style="position: relative; height: 100%; width: 100%; overflow: hidden;" class={className}>
	{#if showPreview}
		<div style="pointer-events: none; position: absolute; right: 12px; bottom: 12px; height: 132px; width: 220px; overflow: hidden; border-radius: 8px; border: 1px solid rgba(255,255,255,0.25); background: black; font-family: sans-serif; font-size: 12px; line-height: 1.2; color: white; box-shadow: 0 4px 16px rgba(0,0,0,0.4);">
			<video bind:this={videoRef} muted playsinline autoplay style="height: 100%; width: 100%; transform: scaleX(-1); object-fit: cover;"></video>
			<div style="position: absolute; top: 8px; left: 8px; border-radius: 6px; background: rgba(0,0,0,0.5); padding: 2px 6px; backdrop-filter: blur(4px);">
