<script>
	...
	// Svelte 5 JavaScript Reactive Props with Fallbacks
	let {
		linesGradient,
	...
	let uniformsRef = $state.raw(null);

	// Dynamically update Three.js shader values when props change at runtime
	$effect(() => {
		const u = uniformsRef;
		if (!u) return;
...
const vec3 PINK  = vec3(0.0);
const vec3 BLUE  = vec3(0.0);
...
void main() { vec4 color = vec4(0.0); mainImage(color, gl_FragCoord.xy); gl_FragColor = color; }`;
        ...
        const renderLoop = () => {
            ...
            // 3. REPLACE THIS CONDITION BLOCK FOR MOUSE LOGIC:
            if (uniforms.interactive.value && targetInfluence > 0) {
                currentMouse.lerp(targetMouse, mouseDamping);
                uniforms.iMouse.value.copy(currentMouse);
                currentInfluence += (targetInfluence - currentInfluence) * mouseDamping;
                uniforms.bendInfluence.value = currentInfluence;
            } else if (uniforms.bendInfluence.value > 0.001) {
                currentInfluence += (0 - currentInfluence) * mouseDamping;
                uniforms.bendInfluence.value = currentInfluence;
            } else {
...
<div bind:this={containerRef} class="absolute inset-0 h-full w-full overflow-hidden" style="mix-blend-mode: normal"></div>

<style>
	/* 1. Force the bounding container div to stay absolute and pinned */
	.absolute {
	...
	/* 💡 2. THE THREE.JS OVERRIDE: Force the canvas to fill the structural layout space */
