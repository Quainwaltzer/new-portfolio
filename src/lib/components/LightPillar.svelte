...
void main() {
  ...
  gl_FragColor = vec4(color * uIntensity, 1.0);
		...
		return () => {
			...
			if (containerRef && containerRef.contains(renderer.domElement)) containerRef.removeChild(renderer.domElement);
...
{#if webGLSupported}
	<div bind:this={containerRef} style="position: absolute; left: 0; top: 0; height: 100%; width: 100%; mix-blend-mode: {mixBlendMode};" class={className}></div>
{:else}
	<div style="position: absolute; left: 0; top: 0; display: flex; height: 100%; width: 100%; items-center: center; justify-content: center; background: rgba(0,0,0,0.1); font-size: 14px; color: #6b7280; mix-blend-mode: {mixBlendMode};" class={className}>
		WebGL not supported
	</div>
{/if}