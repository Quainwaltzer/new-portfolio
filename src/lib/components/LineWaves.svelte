...
void main() {
  ...
  gl_FragColor = vec4(col, alpha);
		...
		// 💡 RE-SIZE OBSERVER RE-PATCH
		const resizeObserver = new ResizeObserver((entries) => {
			for (let entry of entries) {
				const width = entry.contentRect.width || containerRef.offsetWidth;
				const height = entry.contentRect.height || containerRef.offsetHeight;
...
<div bind:this={containerRef} class="aurora-canvas-container"></div>
...
<style>
	...
	.aurora-canvas-container :global(canvas) {
		position: absolute;
		top: 0;
		left: 0;
		width: 100% !important;
